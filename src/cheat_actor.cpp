...
void cheat_handle_maprun(actor_info* self)
{
	if (cheat_state->genel.iMapRun)
	{
		...
		if (KEY_DOWN('W') || KEY_DOWN('A') || KEY_DOWN('D'))
		{
			...
			fPos[0] += sinf(-self->fCurrentRotation) * set.General.MapRun_Speed; // set.MapRun_Speed
			fPos[1] += cosf(-self->fCurrentRotation) * set.General.MapRun_Speed; // set.MapRun_Speed
...
void cheat_handle_actor_autoaim(struct actor_info *info, double time_diff)
{
	...
	//if (KEYCOMBO_PRESSED(set.key_silent_aim_toggle))
	...
	if (!set.use_gta_autoaim)
	{
		...
		if (cheat_state->actor.autoaim && isAimKeyFire)
		{
			...
			// get the head position
			/* RapidFire's Project Attack V1'de 7 tane pozisyon seçebiliyorduk. v1'i paylaþtýðým tarih 20.01.2019
			v2 sürümünde ise hepsini ekleyeceðim. 26.02.2019 12:41
			*/
		
			switch (set.AutoAimPozisyon)
			{
				case 1: // Pelvis1 için
				{
							CVector Pelvis;
							ped->GetTransformedBonePosition(BONE_PELVIS1, &Pelvis);
							vect = pCam->Source - Pelvis;
				}
				break;
				case 2: // Pelvis 2 için
				{
							CVector Pelvis2;
							ped->GetTransformedBonePosition(BONE_PELVIS, &Pelvis2);
							vect = pCam->Source - Pelvis2;
				}
						break;
				case 3: // Spine 1 için
				{
							CVector Spine1;
							ped->GetTransformedBonePosition(BONE_SPINE1, &Spine1);
							vect = pCam->Source - Spine1;
				}
					break;
				case 4: // Uppertorso için
				{
							CVector UpperTorso;
							ped->GetTransformedBonePosition(BONE_UPPERTORSO, &UpperTorso);
							vect = pCam->Source - UpperTorso;
				}
					break;
				case 5: // Neck için
				{
							CVector Neck;
							ped->GetTransformedBonePosition(BONE_NECK, &Neck);
							vect = pCam->Source - Neck;
				}
					break;
				case 6: // BONE_RIGHTUPPERTORSO için
				{
							CVector RightUpTorso;
							ped->GetTransformedBonePosition(BONE_RIGHTUPPERTORSO, &RightUpTorso);
							vect = pCam->Source - RightUpTorso;
				}
					break;
				case 7: // BONE_HEAD1
				{
							CVector boneHead1;
							ped->GetTransformedBonePosition(BONE_HEAD1, &boneHead1);
							vect = pCam->Source - boneHead1;
				}
					break;
				case 8: // BONE_RIGHTSHOULDER
				{
							CVector rightShoulder;
							ped->GetTransformedBonePosition(BONE_RIGHTSHOULDER, &rightShoulder);
							vect = pCam->Source - rightShoulder;
				}
				break;
			default: // Eðer hiç biri açýk deðilse direk kafaya odaklansýn.
				CVector head;
				ped->GetTransformedBonePosition(BONE_HEAD, &head);
		...
		if (KEYCOMBO_PRESSED(set.key_legit_aim_toggle)) // X Tuþu açýyor / X tuþu kapatýyor.
			cheat_state->Silah.iLegitAimBot ^= 1;

		else if (cheat_state->Silah.iLegitAimBot)
		{
...
// keys/buttons input
			...
			if ( !cheat_state->actor.fly_active )
			{
				...
				{
					// start fly animation
					if (set.General.Actor_Fly_Type == 1)
						GTAfunc_PerformAnimation("SWIM", "Swim_Tread", -1, 1, 1, 0, 0, 0, 1, 0);
					else if(set.General.Actor_Fly_Type == 2)
