...
PBYTE ReadRes(char *resN, int &rsize)
{
	...
	hRsrc = FindResourceA(g_hDllModule, resN, RT_RCDATA);
	rsize = SizeofResource(g_hDllModule, hRsrc);
	hGlob = LoadResource(g_hDllModule, hRsrc);

	return (PBYTE)LockResource(hGlob);
}

void Log ( const char *fmt, ... )
{
	if ( !g_szWorkingDirectory ) 
		return;
	...
	{
		char	filename[512];
		snprintf( filename, sizeof(filename), "%s\\" M0D_FOLDER "%s", g_szWorkingDirectory, "project_attack.log" );
	...
	if ( g_flLogAll == NULL )
	{
		...
		snprintf( filename_all, sizeof(filename_all), "%s\\" M0D_FOLDER "%s", g_szWorkingDirectory, "project_attack_all.log" );
...
void LogChatbox ( bool bLast, const char *fmt, ... )
{
	...
	{
		char	filename[512];
		snprintf( filename, sizeof(filename), "%s\\" M0D_FOLDER "%s", g_szWorkingDirectory, "project_attack_chatbox.log" );
	...
	{
		char	filename_all[512];
		snprintf( filename_all, sizeof(filename_all), "%s\\" M0D_FOLDER "%s", g_szWorkingDirectory, "project_attack_chatbox_all.log" );
...
{
	//Log( " ---------------------------------------------------------------------" );
	//Log( " %s çalýþmayý durdurdu.", NAME );
	//Log( " Base address: 0x%p, SA:MP base address: 0x%p", g_hDllModule, g_dwSAMP_Addr );
	//Log( " Projeye son iþlenen fonksiyon: %s", ExceptionInfo->ExceptionRecord->ExceptionAddress, g_szLastFunc );
	...
	case DLL_PROCESS_DETACH:
		if ( g_hOrigDll != NULL )
		{
			...
			if ( set.chatbox_logging && g_SAMP )
				LogChatbox( true, "Döküm sona erdi." );
		}
