...
public:
	...
	virtual void RegisterAsRemoteProcedureCall(int* uniqueID, void(*functionPointer) (RPCParameters *rpcParms, void * param), void * param = 0) = 0;
	virtual void RegisterAsRemoteProcedureCall(RPCEnumeration uniqueID, void(*functionPointer) (RPCParameters *rpcParms, void * param), void * param = 0) = 0;
	...
	virtual void UnregisterAsRemoteProcedureCall(RPCEnumeration uniqueID) = 0;
	...
	virtual bool RPC(int* uniqueID, const char *data, unsigned int bitLength, PacketPriority priority, PacketReliability reliability, char orderingChannel, bool shiftTimestamp) = 0;//, NetworkID networkID, BitStream *replyFromTarget )=0;
	...
	/// \param[in] replyFromTarget If 0, this function is non-blocking.  Otherwise it will block while waiting for a reply from the target procedure, which is remtely written to RPCParameters::replyToSender and copied to replyFromTarget.  The block will return early on disconnect or if the sent packet is unreliable and more than 3X the ping has elapsed.
	/// \return True on a successful packet send (this does not indicate the recipient performed the call), false on failure
	virtual bool RPC(int* uniqueID, RakNet::BitStream *bitStream, PacketPriority priority, PacketReliability reliability, char orderingChannel, bool shiftTimestamp) = 0;//, NetworkID networkID, BitStream *replyFromTarget )=0;
	virtual bool RPC(RPCEnumeration uniqueID, RakNet::BitStream *bitStream, PacketPriority priority, PacketReliability reliability, char orderingChannel, bool shiftTimestamp) = 0;
