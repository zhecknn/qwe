...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::RegisterAsRemoteProcedureCall( int* uniqueID, void ( *functionPointer ) ( RPCParameters *rpcParms , void * param ), void * param )
{
	if ( uniqueID == 0 || uniqueID[ 0 ] == 0 || functionPointer == 0 )
		return;

	rpcMap.AddIdentifierWithFunction(uniqueID, (void*)functionPointer, false, param);
...
void RakPeer::RegisterAsRemoteProcedureCall(RPCEnumeration uniqueID, void(*functionPointer) (RPCParameters *rpcParms, void * param), void * param)
{
	if (uniqueID == 0  || functionPointer == 0)
		return;

	rpcMap.AddIdentifierWithFunction((int *)&uniqueID, (void*)functionPointer, false, param);
...
void RakPeer::RegisterClassMemberRPC( int* uniqueID, void *functionPointer )
{
	...
	rpcMap.AddIdentifierWithFunction(uniqueID, functionPointer, true, NULL);
}
...
bool RakPeer::RPC(int* uniqueID, const char *data, unsigned int bitLength, PacketPriority priority, PacketReliability reliability, char orderingChannel, PlayerID playerId, bool broadcast, bool shiftTimestamp)//, NetworkID networkID, BitStream *replyFromTarget )
{
#ifdef _DEBUG
...
#pragma warning( disable : 4701 ) // warning C4701: local variable <variable name> may be used without having been initialized
#endif
bool RakPeer::RPC(int* uniqueID, RakNet::BitStream *bitStream, PacketPriority priority, PacketReliability reliability, char orderingChannel, PlayerID playerId, bool broadcast, bool shiftTimestamp)//, NetworkID networkID, BitStream *replyFromTarget )
{
	if (bitStream)
...
bool RakPeer::HandleRPCPacket( const char *data, int length, PlayerID playerId )
{
	...
	{
		node->staticFunctionPointer( &rpcParms, node->param );
	}
	else
	{
