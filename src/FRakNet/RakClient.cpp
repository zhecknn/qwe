...
//		}
//        else
//		if ( packet->data[ 0 ] == ID_TIMESTAMP && packet->length == sizeof(unsigned char)+sizeof(unsigned int)+sizeof(unsigned char)+sizeof(unsigned int)+sizeof(unsigned int) )
//		{
//			RakNet::BitStream inBitStream(packet->data, packet->length, false);
//		
//			RakNetTime timeStamp;
...
void RakClient::PingServer(const char* host, unsigned short serverPort, unsigned short clientPort, bool onlyReplyOnAcceptingConnections)
{
		...
		RakPeer::Ping(host, serverPort, onlyReplyOnAcceptingConnections); // RakPeer::Ping( host, serverPort, onlyReplyOnAcceptingConnections );
}
...
void RakClient::RegisterAsRemoteProcedureCall(int* uniqueID, void(*functionPointer) (RPCParameters *rpcParms, void * param), void * param)
{
	RakPeer::RegisterAsRemoteProcedureCall( uniqueID, functionPointer , param);
}

void RakClient::RegisterAsRemoteProcedureCall(RPCEnumeration uniqueID, void(*functionPointer) (RPCParameters *rpcParms, void * param), void * param)
{
	RakPeer::RegisterAsRemoteProcedureCall(uniqueID, functionPointer,param);
}
...
void RakClient::UnregisterAsRemoteProcedureCall(RPCEnumeration uniqueID)
{
	RakPeer::UnregisterAsRemoteProcedureCall((int *)&uniqueID);
}

bool RakClient::RPC(int* uniqueID, const char *data, unsigned int bitLength, PacketPriority priority, PacketReliability reliability, char orderingChannel, bool shiftTimestamp)//, NetworkID networkID), BitStream *replyFromTarget )
{
	if (remoteSystemList == 0)
		return false;
...
bool RakClient::RPC(int* uniqueID, RakNet::BitStream *parameters, PacketPriority priority, PacketReliability reliability, char orderingChannel, bool shiftTimestamp)//, NetworkID networkID, BitStream *replyFromTarget )
{
	if (remoteSystemList == 0)
		return false;
...
bool RakClient::RPC(RPCEnumeration uniqueID, RakNet::BitStream *parameters, PacketPriority priority, PacketReliability reliability, char orderingChannel, bool shiftTimestamp)//, NetworkID networkID, BitStream *replyFromTarget )
{
	if (remoteSystemList == 0)
		return false;

	return RakPeer::RPC((int *)&uniqueID, parameters, priority, reliability, orderingChannel, remoteSystemList[0].playerId, false, shiftTimestamp);//, networkID, replyFromTarget );
}
