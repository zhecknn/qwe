...
stNewModSa						*g_NewModSa = new stNewModSa();
#if COMPILE_SAMP_VERSION == COMPILE_FOR_SAMP037
struct stBotCheats* BotCheats = (struct stBotCheats*)malloc(sizeof(struct stBotCheats));
stPlayerInfo2					g_PlayerInfo[SAMP_MAX_PLAYERS];
#else
stPlayerInfo2					g_PlayerInfo[SAMP_MAX_PLAYERS];
...
#elif COMPILE_SAMP_VERSION == COMPILE_FOR_SAMP03DL
				strcpy(g_szSAMPVer, "SA:MP 0.3DL");
#endif
				Log("%s tanýmlandý. g_dwSAMP_Addr: 0x%p", g_szSAMPVer, g_dwSAMP_Addr);
				sampPatchDisableAnticheat();
				//FPS_Unlocker();
				//AC_AntiWarnings();
				//Log("90 FPS limiti SA-MP'tan kaldýrýldý.");
				//Log("Anti Crasher yüklendi.");
				iIsSAMPSupported = 1;
			}
			else
			{
				Log("Bilinmeyen SA-MP Sürümü.");
				iIsSAMPSupported = 0;
				set.basic_mode = true;
				g_dwSAMP_Addr = NULL;
			}
...
uint8_t _declspec (naked) hook_handle_rpc_packet(void)
{
	...
	static RPCN0de *pRPCNode = nullptr;
...
void installSAMPHooks()
{
	...
	//if (cheat_state->RakNet.BypassSAMPCAC)
	//{
	//	/*SetupSAMPHook("HOOKPOS_OUTGOING_RPC", g_dwSAMP_Addr + HOOKPOS_OUTGOING_RPC, (DWORD)OutgoingRPC, "\x81\xC1\x22\xF2\xFF\xFF",DETOUR_TYPE_JMP);*/
	//}
}
