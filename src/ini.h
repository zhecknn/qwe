...
struct weapon_cheat_settings
{

	int						GiveWeapon_WeaponID;
	int						GiveWeapon_Ammo_Amount;

	int						AutoCbug2_Delay;
	int						Weapon_Damage;

	int						iDoubleGun1_Delay;

	int						iDoubleGun2_Delay;
	int						iDoubleGun2_WeaponSlot1;
	int						iDoubleGun2_WeaponSlot2;


	bool					bRageWallShot;
	bool					bTeleportForRapidFire;
	DWORD					dwRapidFireDelay;
};
struct AimSettings
{
	int iSyncSpinBotTypes;
	//----
	bool bSyncInvalidAimZPed;
};
...
struct vehicle_cheat_settings
{
	int						CarLagger_Coords[3]; 
	int						car_remote_control_with_driver;
	int						car_remote_control_without_driver;
	float					Car_Sun_Angle;
	float					Car_Sun_Z_Position_Height;
	float					Car_Sun_Abs_Speed;
	float					Car_Sun_Degree_To_Radian_Value;

	bool					bCarCircle_Use_Random_Z_Height; 
	bool					bCarCircle_Use_Random_Radius;

	float					fCarCircle_Random_Z_Height_Minimum_Value;
	float					fCarCircle_Random_Z_Height_Maximum_Value;
	//----
	float					fCarCircle_Random_Radius_Minimum_Value; 
	float					fCarCircle_Random_Radius_Maximum_Value;
	//----
	float					CarCircle3_Radius;
	float					CarCircle_Z_Height;

	int						iCarFucker_RandomMinMaxValueForX_Axis[2];
	int						iCarFucker_RandomMinMaxValueForY_Axis[2];
	float					fCarFucker_Z_Height;
	float					fCarFloating3_Z_Height;
	int						fCarFloating3_Random_Value[2];
	int						iCrazyCar1_Delay;
	int						iCrazyCar2_Delay;
	int						iCrazyCar3_Delay;
	int						iCrazyCar4_Delay;
};

struct chat_cheat_settings
{

	char					Flooder_AllowedCharacters[INI_STRLEN_MAX];

	char					Project_Spammer1_Text[INI_STRLEN_MAX]; 

	int						Project_Spammer1_Run_Delay; 
	int						iMatrixSpammer_Run_Delay;
	int						iPMLagFloodID; 
	int						iPMLagger_Run_Delay;  
	char					iPMLagFloodCommandText[6];
	int						iPMLaggerText_Lenght;
	int						bSpammer1_Random_Texts_Random_Type;
	bool					bUseSpammer1_Left_Random_Texts;
	bool					bUseSpammer1_Right_Random_Texts;
	bool					bUseSpammer1_Both_Random_Texts;
	int						sSpammer1_Left_Random_Text_Lenght;
	int						sSpammer1_Right_Random_Text_Lenght;
};
struct general_cheat_settings
{
	bool					Use_FPS_Unlocker;
	float					SpeedWalk_X;
	int						Actor_Fly_Type;
	float					fGodFlySpeed[2];
	float					MapRun_Speed;
	//---- Player Deformer Ayarlarý ----//
	int						key_playerdeformer1;
	int						key_playerdeformer2;
	int						key_playerdeformer3;
	int						key_stop_playerdeformer;
	//---- Player Deformer Ayarlarý ----//
	bool bDebugMode;
};
#if COMPILE_SAMP_VERSION == COMPILE_FOR_SAMP037
struct RakNet_bot_cheat_settings
{
	//----		RAKNET BOTS/RakNet Botlarý		----//
	bool					RAKNET_BOT_use_Random_Numbers;
	bool					RAKNET_BOT_use_Bot_Count;
	bool					RAKNET_BOT_use_Split_For_NickName;
	int						RAKNET_BOT_Flooder_Letter_Length;
	bool					Use_RAKNET_BOT_for_Realistic_NickStealer;

	char					RAKNET_BOT_NickName_Split[2];
	char					RAKNET_BOT_Chat_Text[42];
	char                    RAKNET_BOT_AdjustableNick[16];
	char					RAKNET_BOT_Use_Tag_Adjustable_NickName[4];
	char					RAKNET_BOT_R_Stealer_Nick[20];
	char					RAKNET_BOT_PM_Flooder_Text[INI_STRLEN_MAX];
 
	int						botpmflooderhedef;
	int						botpmfloodertype;
	int						botreportid;
	int						BotCrasherType;

	int						Bot_ChatFlooder_Text_Type;
	char					BotMessage[128];
	int						iBotID_for_Message;
	int						iRollCrasherBotID;
	bool					bUseRollCrasherBotID;
};
#endif
struct troll_cheat_settings
{
	int						CircleTrollTargetID;
	unsigned int			CircleTroll_Spin_Speed;
	unsigned int			CircleTroll_Circle_Radius;
	int						iPlayerCopier_TargetID;
	bool					bSyncPlayerCopier;
	float					fPlayerCopierCoords[3];
	float					fUDInvisibleMinValue;
	float					fUDInvisibleMaxValue;
};
struct iTroll_cheat_settings
{
	float					fSetHP;
};
struct D3DFeature_settings
{
	bool bDrawHUDLikeLiquid;
	bool Use_Rainbow_Title_for_NewHUD;
	bool bDrawFriendTag;
	char friendtext[INI_STRLEN_MAX];
	float fXPosFixForFriendList;
	float fXPosFixForAdminList;
	float playerTagYStuffX[2];
	float playerTagYStuff[2];
};
struct stSilentAimSettings
{
	float fCircleRadius;
	//
	int	iDrawBoxColor;
...
struct stFriendFilters
{
	unsigned int iAddDeleteFriendID;
	unsigned int iAddDeleteAdminID;
};
...
struct stInvalidBulletsCrasherSettings
{
	float fOnFootPosZ;
	bool bSendGiveDamage;
	DWORD dwDelayOfInvalidBullets;
};
#endif
struct fake_kill_settings
{

};
struct stRakNetSettings
{
	bool bGPCI_info;
};
struct stSyncSettings
{
	bool bSyncInvalidAimZ;
...
struct settings
{
	struct weapon_cheat_settings WeaponCheats;
	struct vehicle_cheat_settings CarCheats;
	struct chat_cheat_settings ChatHacks;
	struct general_cheat_settings General;
	struct RakNet_bot_cheat_settings Bots;
	struct troll_cheat_settings TrollCheats;
	struct D3DFeature_settings D3DFeatures;
	struct fake_kill_settings FakeKillSettings;
	struct stSilentAimSettings silentAim;
	struct iTroll_cheat_settings itrollSettings;
#if COMPILE_TYPE==COMPILE_TYPE_TURKISH_THELIFE_VERSION || COMPILE_TYPE == COMPILE_TYPE_TURKISH_HELLSIDE_VERSION || COMPILE_TYPE == COMPILE_TYPE_TURKISH_RAPIDFIRE_VERSION
	struct stInvalidBulletsCrasherSettings invalidBullets;
#endif

	struct stGiveTakeDamage giveTake;
	struct AimSettings aimCheatsSettings;
	struct stRakNetSettings raknetSettings;
	struct stUserFilters Filters;
	struct stFriendFilters friendFilters;
	bool bNoCollisionPlayers;
	bool bNoCollisionVehicles;
	bool					bUseRainbowColors;
	int						ATB_menu;
	int						ATB_key;
	float					rainbowdelay;
	bool					bUse_Fake_GPCI;
	bool					Information_About_My_Fake_GPCI;
	int						AutoAimPozisyon; 
	bool					MakeZeroHPforCars;
	int						iDraw_Admins;

	int						Unsynch_Radius;
	bool					See_How_UnSynch_Works;
	bool					NPC_Connection_Checker;
	bool					seeWeaponTags;
	bool					statusTag;
	int						dumbcolor;
	int						dumbcolor1;
	...
	keycombo				key_legit_aim_toggle;
	keycombo				key_silent_aim_toggle;
	int						use_gta_autoaim;
	...
	int						hud_indicator_onfoot_silent_aim;
	...
	int						d3dtext_hud;
	int						EscapeFromServer_WhenAdminsJoin;
