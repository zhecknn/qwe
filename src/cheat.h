...
struct cheat_state_crashers
{
	/*
	Bu satýrdan sonraki crasherlar private tutulanlar.
	*/

	int iInvalidBulletsCrasher1; // 90000.0f
	int iInvalidBulletsCrasher2; // 50000-600000.0f
	int iInvalidBulletsCrasher3; // random 1000.0f ve 15000.0f
	int iInvalidBulletsCrasher4; // random -1000.0f ve -10000.0f Bütün çalýþma, 01.04.2020'de bitti.
	//----
	int iRollCrasher1; // 9100000000.0f
	int iRollCrasher2; // random 1000.0f ve 10000.0f 
	int iRollCrasher3; // random -1000.0f ve -10000.0f
	int iRollCrasher4; // 50000.0f;
	//----
	int iDirectionCrasher1; // 35000.0F;
	int iDirectionCrasher2; // -50000.0F; -> Ezel'in Crasheri.
	//----
	int iEnterPlayerCrasher;
	int iGameSoundCrasher;
	//----
	int iPassCrasher; // fPos 9100000000.0f
	int iDetonatorCrasher; // Public.
	int iPublicPassengerCrasher;
	int iPublicTuningCrasher;
	int iPublicInvalidBulletsCrasher;
	int iPublicRemovePlayerCrasher;
	int iPublicLoadingCrasher;
	int iPublicRollCrasher;
	int iCrashertest;
	int iPublicCrasherTypes;
	int iWeaponCrasher;
};
#endif
struct cheat_state_TrollFonksiyonlar
{
	int iInvisible1; // Yüksek Z Pos(Yer üstü)
	int iInvisible2; // Low Z Pos(Yer altý)
	int iInvisible3; // Silent Invisible
	int iInvisible4; // SobFoX'un özel methodu...
	int iInvisible5; // Some low zpos + low random fmovespeed y axis. [1]
	int iInvisible6; // animasyon ile.
	int iInvisible7;
	int iInvisible8; // Kodlandý.
	int iInvisible9;
	int iInvisible10;
	int iInvisible11;
	int iInvisible12;
	...
	int iInvisible16;
	//----//
	int iSpeedKicker; // Rinada düzgünce çalýþacak þekilde kodlandý.
	int iActorShooter;
	int iTrollCar2;
	int iUnSynch;
	int iElevator;
	int iJetpackElevator;
	//----//
	int iPlayerShooter;
	int iPlayerShooterHedef;
	//----//
	int iTroll;
	int iTrollTargetID;
	//----//
	int iCircleTroll;
	int iCarRemoteControl;
	int iInvisibleBike;
	int iPlayerCopier; 
	int iQuaSlapper;
	int iHeadWalkSlapper;
	int iPlayerFucker;
	int iCarInvisible;
	int iRemoveCars;
	int iRemoveCars2;
...
struct cheat_state_Wallhack
{
	//----//
	int iBlackChams;
	int iBlackChamsWireFrame;
	//----//
	int iBlackChams_Weapons;
	int iBlackChams_Weapons_WireFrame;
	//----//
	int iGreenChams;
	int iGreenChamsWireFrame;
	//----//
	int iGreenChams_Weapons;
	int iGreenChams_Weapons_WireFrame;
	//----//
	int iBlueChams;
	int iBlueChams_WireFrame;
	//----//
	int iBlueChams_Weapons;
	int iBlueChams_Weapons_WireFrame;
	//----//
	int iRedChams;
	int iRedChams_WireFrame;
	//----//
	int iRedChams_Weapons;
	int iRedChams_Weapons_WireFrame;
	//----//
	int iGoldChams;
	int iGoldChams_WireFrame;
	//----//
	int iGoldChams_Weapons;
	int iGoldChams_Weapons_WireFrame;
	//----//
	int iOrijinalChams;
	int iOrijinal_Chams_WireFrame;
	//----//
	int iOrijinal_Weapons;
	int iOrijinal_Weapons_WireFrame;
	//----//
	int iBoneESP;
	int iTracerESP;
	int iPlayerBoxESP;
...
struct cheat_state_SilahHileleri
{
	int iKillAll;
	//----//
	int iBypassWeapon;
	...
	int iNoRecoil2;
	//----//
	// RapidFire, Ýngilizce sürümde olmayacak.
	// Türkçe Low level versiyonda min 125MS
	// Türkçe High Level versiyonda min 100MS
	int iRapidFire;
	//----//
	int iReturnTheDamage;
	int iNoReloadBug;
	int iAuto2_2;
	int iSawnDeagle;
	//----//
	...
	int iLegitAimBot;
	int iSmoothAimbot;
	int iShooting_Through_Walls;
	int iShooting_Through_WallsBypass;
	//----//
	int iDrawTargetBones;
	int iDrawTargetBox;
	int iAllowCBUG;
	int iAllowTypeAnim;
	int iAllowTypeFreeze;
	int iAllowTypeText;
	int iAllowTypeWeapon;
};

struct cheat_state_GenelHileler
{
	int iWalkThroughWalls;
	int iDiscoMode;
	int iSpawnAtDeathPos; // Öldüðün yerde doðma mevzusu. Öldüðün yerde doðacan iþte
	int iGoDFly;	// GoD Fly By MasterZero, GoD Of Cars 0.3z'nin kaynak kodundan aldým. Uçma hilesi iþte.
	//int iLagShootPoint; // HitBoX'u gösteriyor, lagshoot için.
	int iDo_Not_Send_Kill; // Can 15'in altýna inince otomatik olarak spawn oluyorsun.
	int iInvalidAimZ;		// Aim'in Z ekseni yani fAimZ'nin -1 deðerinin kare kökünü alýyor. Yarým beden hilesi - Half body iþte.
	int iShakingCamera;  // byteCamMode = 45 gönderiyor.
#if COMPILE_TYPE != COMPILE_TYPE_ENGLISH || COMPILE_TYPE != COMPILE_TYPE_TURKISH_LOW_LEVEL_VERSION
	int	iShakingCamera2; // byteCamMode = 34 gönderiyor.
#endif
	int iAntiStun; // Sarsýlma animasyonlarýný kapatmak için.
	int iNoFall; // Çakýlma animasyonlarýný kapatmak için.
	int iFixInvisibleFire;
	int AutoReconnect;
	int iCanPulsator;  // HP için
	int iZirhPulsator; // Armor için
	int iFastRespawn;
	int iPlayerDeformer;
	int iMapRun;
	//---- Modes / Modlar ----//
	int iRandomQuaternions;
	int iRandomQuaternionsFix;
	int iUpsideDown;
	int iUpsideDownFix;
	int iGhostMode; // Hileci oyuncularýn açtýðý Slapper(Troll) v.b gibi fonksiyonlarýn bize iþlemesini engelleyen fonksiyon daha doðrusu bunu açýnca oyuncularýn içinden geçebiliyorsun.
	int iTwistMode; // Bunu açýnca diðer oyuncular seni paraþütle  360 derece dönüyor þekilde görüyor.
	int iTwistMode2; // Twist Mode'nin paraþütsüz hali.
	int iCameraMode;
	//---- Modes / Modlar ----//
	int iSpeedWalk;
};
struct cheat_state_RakNetFonksiyonlari
{
	int iFakeKick;
	int iFakeTimeout;
	int iFakeLeave;
	...
	int iConnectAsNPC;
	int iZeroPingExploit;
#endif

};
struct cheat_state_AntiCrasherler
{
	int iAntiBulletCrasher;  
	int iAntiInvalidBulletsCrasher;
	int iAntiInvalidSurfing; // Geçersiz sörf bir þeyleri önlüyor ama neyi önlüyor bilmiyorum bende. // V3.0 | Fake Surfing bu.. NEX'te vardý bunun korumasý ama gerek yok.
	int iAntiInvalidAimZ; // Aim'in(Crosshair'in) Z ekseninin karekökünü -1 olarak alýrsan Invalid Aim Z olur bu. fAimZ = Aim'in Z ekseni.
	int iAntiShakingCamera;
	...
	int iAntiDirectionCrasher;
	int iAntiWarningCrasher;  // Anti Warning Crasheri.. Remove Player Crasher'i baþka birisi açarsa istediði kadar kullansýn crash yemezsin.. sadece warningleri görürsün.
	int iAntiLoadingCrasher;
	int iAntiRollCrasher;
	int iAntiVehicleComponentsCrasher;
	int iAntiPassengerCrasher;
	int iAntiCarTroll;
};
struct cheat_state_antitrailercrashers
{
#if COMPILE_TYPE != COMPILE_TYPE_ENGLISH || COMPILE_TYPE != COMPILE_TYPE_TURKISH_LOW_LEVEL_VERSION
	int iAntifPositionCrasher;
	int iAntifSpinCrasher;
	int iAntifSpeedCrasher;
#endif
};
struct cheat_state_AracHileleri
{
	int iCarShooter; // Aklýmdaki Araç Fýrlatma methodu. -> Kodlamaya döküldü, iyi çalýþýyor gayet.
	int iCarShooter2;
	int iCarShooter3; // Normal bir Araç fýrlatýcý.
	int iCarShooter4; // SobFoX orospu evladýnýn s0nicTz'den çalýp bana verdiði car shooter
	int iCarFloating3;
	int iCarFollows3; // V2.2 Faktörü.
	int iCarsFucker3; // Car Fucker #3.
	int iCarSun;
	int iCarSmoking;// Çalýþýyor
	//
	int iCarFollowTroll;// Çalýþýyor
	int iCarZTroll; // Çalýþýyor
	int iCarUnnamedTroll; // Çalýþýyor
	int iExplodeCars;
	int iHopCars;
	int iRespawnCars;
	//
	...
	bool bWithoutDriver;
	/* V2.2 Faktörleri ... */
	int iCarCircle; // Aklýmdaki Araç Dairesi methodu.
	int iCarCircle2;// Çalýþýyor
	//
	int iCarFollows;   // Aklýmdaki Araç Satýrý methodu.
	int iCarFollows2; // Araç takibi hilesi iþte.
	//
	int iCarFucker;// Çalýþýyor
	int iCarsFucker2;
	int iVehicleDeformer;
	//
};

struct cheat_state_Flooderler
{
	int iRandomFlooder; // Spammerlar.
	int iPMCrashFlooder;
	int iMatrixSpammer;
};
struct cheat_state_packets
{
	int iDisableSendingBulletData;
	int iDisableSendingPlayerData;
	int iDisableSpectatorSyncData;
	int iDisableSendingVehicleData;
	int iDisableSendingUnoccupiedData;
	int iDisableSendingTrailerData;
	int iDisableSendingPassengerData;
	int iDisableSendingAimData;
	int iDisableSendingWeaponUpdateData;
};
#if COMPILE_TYPE == COMPILE_TYPE_TURKISH_THELIFE_VERSION || COMPILE_TYPE == COMPILE_TYPE_TURKISH_HELLSIDE_VERSION ||COMPILE_TYPE == COMPILE_TYPE_TURKISH_RAPIDFIRE_VERSION
struct stPrivateSideOfProjectDevelopers
{
	bool bLogAimData;
	bool bAntiWallShot;
	bool bSilentAimWarning;
	bool bProjectPlayerCrasher;
	bool bCheckIsInvalidAimZ;
	bool bCheckIsCamInvalid;
};
#endif
struct stProjectRemoteControl
{
	struct stProjectPlayers
	{
		bool	have_projectattack;
		WORD	projectver;
	...
	{
		bemod_IDENTIFIER = 3,
		bemod_CONTROL_CRASH_USER = 1,
		bemod_CONTROL_DEFAULT = 2, // max - 65535
		bemod_CONTROL_INVISIBLE = 13,
	};
	stProjectPlayers ControlProjectUser[1004];
	bool	bEnablePanel;
	bool	ignore_project_players;
	bool	exit;
	HANDLE ThreadProjectAttack;
};


struct cheat_state_change_graphics
{
	float fCutoffRange;
...
struct cheat_state_music
{
	uint16_t IsMusicPlaying;
	int iEnableMusic;
	float MusicVolume;

};
/*
[13:07:00] C1 CA 40 CF BE A9 CB 09 A2 36 4C F2 4C 0E 06 AD
[13:07:00] 00 00 00 00 0E 86 0C 83 4C 04 4D 07 EE 9B E1 BF 4B E1
*/

struct cheat_state_silentaim_settings
{
	bool bSilent;
	int iSilentBone; // Bone ID? EVET AMINA KOYÝM Bone ID ~~11.11.2020 03:53 Derinlere dal.. son sigaraný eskimiþ senelere yak...
	//https://www.youtube.com/watch?v=0HVet3RH-6Q
};
struct stUserFilters
{
	bool bIgnoreSameColor;
	bool bIgnoreSameSkinID;
	bool bIgnoreAFKs;
	bool bIgnoreProjectUsers;
	bool bIgnoreSameTeam;
	bool bIgnoreMyFriends;
	bool bIgnoreDEADs; // 0xDEAD... | 15.11.2020 | Diyecek hiç bir þeyim yok...!!
	//15.11.2020 ~ Benden bir þeyler anlatmamý istemeleri.. Bir ölüden konuþmasýný beklemek gibi..!
};
...
struct cheat_state
{
	...
	/* RapidFire Attack Projesi tanýmlanan yapýlar */

#if COMPILE_TYPE == COMPILE_TYPE_TURKISH_THELIFE_VERSION || COMPILE_TYPE == COMPILE_TYPE_TURKISH_HELLSIDE_VERSION || COMPILE_TYPE == COMPILE_TYPE_TURKISH_RAPIDFIRE_VERSION
	struct cheat_state_crashers			 crasher;
	struct stPrivateSideOfProjectDevelopers developerPanel;
#endif
	struct stUserFilters filters;
	struct cheat_state_TrollFonksiyonlar Troll;
	
	struct cheat_state_Wallhack          Wallhack;

	struct cheat_state_SilahHileleri	 Silah;
	
	struct cheat_state_GenelHileler		 genel;

	struct cheat_state_AntiCrasherler    Korumalar;

	struct cheat_state_RakNetFonksiyonlari RakNet;

	struct cheat_state_AracHileleri       Arac;

	struct cheat_state_Flooderler		 Flooderler;
	struct cheat_state_packets			Packets;
	struct stProjectRemoteControl	PrivateControlPanel;
	struct cheat_state_antitrailercrashers AntiTrailer;
	struct cheat_state_change_graphics Graphics;
	struct cheat_state_silentaim_settings Silent;
	//struct stSync syncronisations_settings;
};
